...
export function End() { const navigate = useNavigate(); const location = useLocation(); const state = location.state as EndState | null
  ...
  useEffect(() => {
    const denied = !isPending && Boolean(state?.ownerId && state.ownerId !== effectiveOwnerId)
    if (denied && state) navigate('/end', { replace: true })
  }, [effectiveOwnerId, isPending, navigate, state])
  if (isPending && navigator.onLine && workspace.kind !== 'guest') return <main className="grid min-h-screen place-items-center p-6 font-semibold">Loading account…</main>
  if (!session?.user && workspace.kind === 'account' && navigator.onLine) return <main className="grid min-h-screen place-content-center gap-4 p-6 text-center"><p>Sign in to view your results.</p><Button onClick={() => void authClient.signIn.social({ provider: 'google', callbackURL: '/end' })}>Sign in again</Button></main>
  if (!state?.ownerId || state.ownerId !== effectiveOwnerId) return <main className="grid min-h-screen place-content-center gap-4 p-6 text-center"><p>These results are not available for this workspace.</p><Button onClick={() => navigate('/start', { replace: true })}>Go to my modules</Button></main>
