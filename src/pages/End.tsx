...
interface EndState { results: QuizResult; mode: 'practice' | 'exam'; questions: Question[]; answers: (number | number[] | string | null)[]; modules?: Module[]; randomize?: boolean; questionLimit?: number; distributionMode?: 'equal' | 'proportional'; timerDuration?: number }
...
export function End() { const navigate = useNavigate(); const location = useLocation(); const state = location.state as EndState | null
  if (!state?.results) return <main className="grid min-h-screen place-content-center gap-4 text-center"><p>No results available.</p><Button onClick={() => navigate('/start')}>Go to setup</Button></main>
  ...
  const retry = () => { if (!modules?.length) { sessionStorage.removeItem('jayawijaya-running'); sessionStorage.removeItem('jayawijaya-quizstate'); navigate('/start'); return } const runningState = { modules, mode, randomize: state.randomize ?? false, questionLimit: state.questionLimit, distributionMode: state.distributionMode, timerDuration: state.timerDuration, timerStart: (state.timerDuration ?? 0) > 0 ? Date.now() : undefined }; sessionStorage.setItem('jayawijaya-running', JSON.stringify(runningState)); navigate('/running', { state: runningState }) }
  return <PageShell className="max-w-4xl"><PageHeader title="Results" actions={<><Button variant="outline" onClick={() => navigate('/start')}><ArrowLeft /> Setup</Button><Button onClick={retry}><RotateCcw /> Retry</Button></>} />
    ...
    <div className="flex gap-3"><Button className="flex-1" variant="outline" onClick={() => navigate('/start')}><ArrowLeft /> Setup</Button><Button className="flex-1" onClick={retry}><RotateCcw /> Retry</Button></div>
  </PageShell>
}
