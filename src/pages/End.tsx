...
export function End() { const navigate = useNavigate(); const location = useLocation(); const state = location.state as EndState | null
  ...
  const retry = () => { if (!modules?.length) { if (state.ownerId) removeQuizSnapshot(state.ownerId); sessionStorage.removeItem('jayawijaya-active-owner'); navigate('/start'); return } if (!state.ownerId) { navigate('/start'); return } const runningState: RunningState = { ownerId: state.ownerId, modules, mode, randomize: state.randomize ?? false, questionLimit: state.questionLimit, distributionMode: state.distributionMode, timerDuration: state.timerDuration, timerStart: (state.timerDuration ?? 0) > 0 ? Date.now() : undefined }; navigate('/running', { state: runningState }) }
