...
interface Props {
  ...
  onPublish: (id: string, name: string, moduleIds: string[], expectedVersion: number) => Promise<LiveCategory>;
  onSetSharing: (id: string, enabled: boolean) => Promise<LiveCategory>;
}

export function LiveCategoryActions({ category, liveCategory, disabled, onCreate, onPublish, onSetSharing }: Props) {
  ...
  const run=async()=>{ setBusy(true); try {
    setError('');
    let live=liveCategory ? await onPublish(liveCategory.id,category.name,category.moduleIds,liveCategory.currentVersion) : await onCreate(category.name,category.moduleIds,category.id);
  ...
  return <><Button size="sm" variant="outline" disabled={disabled||busy||category.moduleIds.length===0} onClick={()=>void run()}>{liveCategory?<RefreshCw/>:<Radio/>}{busy?'Publishing…':liveCategory?'Publish update':'Share live'}</Button>
