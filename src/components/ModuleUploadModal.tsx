...
import { computeFileHash, parseModule } from "@/lib/parser"
import { Alert, AlertDescription } from "@/components/ui/alert"
import { Button } from "@/components/ui/button"
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@/components/ui/dialog"
import { Textarea } from "@/components/ui/textarea"

interface Props { open: boolean; onClose: () => void; onUpload: (modules: Module[]) => void; existingModules: Module[] }
function validateYAML(content: string): string | null {
  if (!content.trim()) return 'No content provided.'
  let parsed: unknown; try { parsed = load(content) } catch (e) { return `YAML parse error: ${e instanceof Error ? e.message : String(e)}` }
...
export function ModuleUploadModal({ open, onClose, onUpload, existingModules }: Props) {
  const fileRef = useRef<HTMLInputElement>(null); const [yaml, setYaml] = useState(''); const [error, setError] = useState(''); const [duplicate, setDuplicate] = useState(''); const [busy, setBusy] = useState(false)
  ...
  const finish = useCallback(async () => { const validation = validateYAML(yaml); if (validation) { setError(validation); return } setBusy(true); setDuplicate(''); try { const hash = await computeFileHash(yaml); const module = parseModule(yaml, `pasted-${Date.now()}`); module.hash = hash; const dup = existingModules.find(m => m.hash === hash || m.title.toLowerCase() === module.title.toLowerCase()); if (dup) { setDuplicate(dup.hash === hash ? `File already uploaded: ${dup.title}` : `Module with same title exists: ${module.title}. Please rename.`); return } onUpload([module]); onClose() } catch (e) { setError(`Failed to process module: ${e instanceof Error ? e.message : String(e)}`) } finally { setBusy(false) } }, [existingModules, onClose, onUpload, yaml])
