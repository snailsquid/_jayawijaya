...
interface Props { open: boolean; onClose: () => void; onUpload: (modules: Module[]) => void | Promise<void>; existingModules: Module[]; replacementFor?: Module; mode?: 'file' | 'code' }
...
export function ModuleUploadModal({ open, onClose, onUpload, existingModules, replacementFor, mode = 'code' }: Props) {
  const fileRef = useRef<HTMLInputElement>(null); const [yaml, setYaml] = useState(() => replacementFor ? moduleToYAML(replacementFor) : ''); const [error, setError] = useState(''); const [duplicate, setDuplicate] = useState(''); const [busy, setBusy] = useState(false); const [live, setLive] = useState(replacementFor?.visibility === 'live')
  ...
  return <Dialog open={open} onOpenChange={value => { if (!value) onClose() }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>{replacementFor ? `Edit ${replacementFor.title}` : mode === 'file' ? 'Upload YAML file' : 'Create module from code'}</DialogTitle><DialogDescription>{replacementFor ? 'Edit the YAML source or change whether this module is shared live.' : mode === 'file' ? 'Choose a YAML file, review it, then create the module.' : 'Paste or write YAML code to create a module.'}</DialogDescription></DialogHeader>
    <input ref={fileRef} type="file" accept=".yaml,.yml" onChange={choose} className="sr-only" aria-label="Choose YAML file" />
    {(mode === 'file' || replacementFor) && <Button variant="outline" onClick={() => fileRef.current?.click()}><FileUp /> Choose YAML file</Button>}
    ...
    <Label className="flex items-center gap-2"><Checkbox checked={live} onCheckedChange={value => setLive(value === true)} /> Share as a live module</Label>
