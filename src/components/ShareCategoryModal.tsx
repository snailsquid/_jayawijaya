...
interface Props { category: LiveCategory; url: string; onClose: () => void; onCopied?: (message: string) => void }
const copy = async (value: string, done?: (message: string) => void, message = 'Copied.') => { await navigator.clipboard.writeText(value); done?.(message); };

export function ShareCategoryModal({ category, url, onClose, onCopied }: Props) {
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent>
    ...
    <DialogFooter className="sm:justify-start"><Button variant="outline" onClick={() => void copy(url,onCopied,'Category link copied.')}><Link /> Copy URL</Button>
      <Button variant="outline" disabled={!category.shareCode} onClick={() => category.shareCode && void copy(category.shareCode,onCopied,'Category code copied.')}><Hash /> Copy code</Button></DialogFooter>
