...
interface Props { category: LiveCategory; url: string; onClose: () => void; onDisable?: () => Promise<void> }
...
export function ShareCategoryModal({ category, url, onClose, onDisable }: Props) {
  ...
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent>
    ...
    <DialogFooter className="sm:justify-start"><Button variant="outline" onClick={() => void copy(url,'Category link copied.')}><Link /> Copy URL</Button>
      <Button variant="outline" disabled={!category.shareCode} onClick={() => category.shareCode && void copy(category.shareCode,'Category code copied.')}><Hash /> Copy code</Button>
      {onDisable && <Button variant="destructive" disabled={disabling} onClick={() => { setDisabling(true); void onDisable().catch(() => undefined).finally(() => setDisabling(false)); }}><RadioTower /> {disabling ? 'Disabling…' : 'Disable live category'}</Button>}
    </DialogFooter>
